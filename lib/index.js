...
exports.jwtEncode = exports.jwtDecode = undefined;
...
var jwtDecode = exports.jwtDecode = function jwtDecode(jwt) {
  try {
    ...
    var base64URLDecodedHeader = Buffer.from(header).toString("utf8");
    ...
    if (components.length === 3) {
      ...
      var base64urlDecodedPayload = Buffer.from(payload).toString("utf8");
...
var hs256Sign = function hs256Sign(headerPayload, key) {
  var secret = _crypto2.default.createSecretKey(key, "base64url");
...
var rs256JWKSign = function rs256JWKSign(headerPayload, key) {
  var hashes = _crypto2.default.getHashes();
  if (hashes.includes("RSA-SHA256")) {
    var secret = void 0;
    try {
      secret = JSON.parse(key);

      var keyObject = _crypto2.default.createPrivateKey({ key: secret, format: "jwk" });
      var sig = _crypto2.default.sign("sha256", Buffer.from(headerPayload), {
        key: keyObject
      });
...
var rs256PEMSign = function rs256PEMSign(headerPayload, privateKey) {
  ...
  var pemKey = _crypto2.default.createPrivateKey({ key: privateKey, format: "pem" });
  var sig = _crypto2.default.sign("sha256", Buffer.from(headerPayload), {
    key: pemKey
  });
  var sigBase64URL = sig.toString("base64url");
...
var rs256PEMVerify = function rs256PEMVerify(headerPayload, publicKey, signature) {
  var keyObject = _crypto2.default.createPublicKey({
    key: publicKey,
    format: "pem"
  });
  var isVerified = _crypto2.default.verify(null, Buffer.from(headerPayload), {
    key: keyObject
 ...
 */var jwtEncode = exports.jwtEncode = function jwtEncode(header, payload, privateKey, keyFormat) {
