...
export const jwtDecode = jwt => {
  try {
    ...
    const base64URLDecodedHeader = Buffer.from(header, "base64url").toString("utf8");
    ...
    if (components.length === 3) {
      ...
      const base64urlDecodedPayload = Buffer.from(payload, "base64url").toString("utf8");
...
export const rs256JWKVerify = (jwt, publicKey) => {
  ...
  const isVerified = crypto.verify(null, Buffer.from(headerPayload, "ascii"), {
    key: keyObject
